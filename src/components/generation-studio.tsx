...
type Gen = { id: string; kind: string; version: number; format: string; createdAt: string };
    ...
    <div>
      ...
      {items.length === 0 ? (
        ...
        <ul className="flex flex-col gap-2">
          {items.map((g) => (
            <li key={g.id} className="flex items-center justify-between rounded-md border border-border bg-surface px-3 py-2">
              <span className="text-sm">v{g.version} · {g.format} · {new Date(g.createdAt).toLocaleDateString()}</span>
              <div className="flex gap-1">
                <Button asChild size="sm" variant="ghost"><a href={`/api/pdf/${g.id}`} target="_blank" rel="noreferrer"><ExternalLink className="size-4" /></a></Button>
                <Button asChild size="sm" variant="ghost"><a href={`/api/pdf/${g.id}`} download><Download className="size-4" /></a></Button>
              </div>
            </li>
          ))}
        </ul>
